...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        if (isset($_POST['itemName']) && isset($_POST['quantity']) && isset($_POST['totalPrice'])) {
            ...
            $stmt = $conn->prepare("INSERT INTO commande (num_table, order_details, total_prix) VALUES (:numTable, :orderDetails, :totalPrice)");
            $stmt->bindParam(':numTable', $numTable);
            $stmt->bindParam(':orderDetails', $orderDetails);
            $stmt->bindParam(':totalPrice', $totalPriceOfOrder);

            // Numéro de table statique (à modifier selon vos besoins)
            $numTable = 1;
